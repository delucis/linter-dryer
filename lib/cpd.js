...
async function detectCopyPaste (textEditor) {
  ...
  const opts = {
    path: scope === 'project' ? atom.project.getPaths()[0] : path.dirname(editorPath),
    files: scope === 'project' ? '**/*' : path.basename(editorPath),
    exclude,
    reporter: 'json',
    'min-lines': minLines,
...
/**
 * Returns the line length for `row` in the file at `path`
 * @param  {String} path      Path to the file to load
 * @param  {Number} row       Number of the row to fetch the length of
 * @return {Promise<Number>}  Length of the row in the file
 */
async function getRangeEndCol (path, row) {
  const buffer = await TextBuffer.load(path)
  return buffer.lineLengthForRow(row)
}
...
function buildExcerptMessage ({ lines, firstFile, secondFile }) {
  let msg = `L${firstFile.start}–${firstFile.start + lines - 1}: Duplicate found at L${secondFile.start}–${secondFile.start + lines - 1}`
      ...
      [[firstFile, secondFile], [secondFile, firstFile]].map(
        async ([firstFile, secondFile]) => {
          ...
          if (firstFile.name === textEditor.getPath()) {
            ...
            rangeEndCol = await getRangeEndCol(firstFile.name, rangeEndRow)
          }
